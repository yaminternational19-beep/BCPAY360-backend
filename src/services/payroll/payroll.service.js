...
export const getEmployeesForPayroll = async (
  ...
  /* ===================================
     4️⃣ FIXED LEAVE CALCULATION (Month Overlap)
  =================================== */
  ...
  /* ===================================
     5️⃣ PAYROLL STATUS (Page Safe)
  =================================== */

  const [payrollPage] = await db.query(
    `
    SELECT
      pee.employee_id,
      pee.payment_status
    ...
    WHERE pb.company_id = ?
      AND pb.pay_month = ?
      AND pb.pay_year = ?
      AND pee.employee_id IN (${placeholders})
  `,
    [companyId, month, year, ...empIds]
  );

  const attMap = toMap(attendance);
  const leaveMap = toMap(leaves);
  const payMap = toMap(payrollPage);
  ...
  const summary = {
    total,
    paid: payrollPage.filter(p => p.payment_status === "PAID").length,
    pending: payrollPage.filter(p => p.payment_status === "PENDING").length,
    not_generated: total - payrollPage.length
  };

  /* ===================================
     FINAL EMPLOYEE DATA (Signed URL)
  =================================== */

  const paginatedEmployees = await Promise.all(
    employees.map(async (emp) => {
      ...
      return {
        ...
        account_number: emp.account_number || "NA",
        ifsc_code: emp.ifsc_code || "NA",
        profile_image_url: emp.profile_photo_path
          ? await getS3SignedUrl(emp.profile_photo_path, 259200)
          : null,
        payment_status: pay ? pay.payment_status : "NOT_GENERATED"
      };
    })
